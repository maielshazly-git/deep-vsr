...
static const arg_def_t saveimgsarg =
        ARG_DEF(NULL, "save-imgs", 1, "Save images upon measuring quality of non per-frame sr method");
static const arg_def_t saveimgsdirarg =
        ARG_DEF(NULL, "save-imgs-dir", 1, "Directory in which images are saved upon measuring quality of non per-frame sr method");
...
{   &help, &codecarg, &use_yv12, &use_i420, &flipuvarg, &rawvideo, &noblitarg, &progressarg, &limitarg, &skiparg,
    ...
    &postfixarg, &dnnruntimearg, NULL, &saveimgsarg, &saveimgsdirarg};
  ...
  vpx_image_t *img_shifted = NULL;
    ...
    for (argi = argj = argv; (*argj = *argi); argi += arg.argv_step)
    {
      ...
      ec_enabled = 1;
        ...
        else if (arg_match(&arg, &saveimgsarg, argi))
            nemo_cfg->save_imgs = 1;
        else if (arg_match(&arg, &saveimgsdirarg, argi))
            nemo_cfg->save_imgs_dir = arg.val;
        else
            argj++;
    }
