...
static void save_sr_quality(VP9_COMMON *cm) {
    ...
    printf("output,%d frame: %.4fdB\n", cm->current_video_frame - 1, psnr_stats.psnr[0]);
#endif
    // Mai, final task, added to save images after sr
    if (cm->nemo_cfg->save_imgs == 1)
    {
        YV12_BUFFER_CONFIG *scaled_rgb_frame = cm->rgb24_reference_frame;
        RGB24_realloc_frame_buffer(scaled_rgb_frame, width, height);
        YV12_to_RGB24(scaled_rgb_frame, sr_upscaled_frame, cm->color_space, cm->color_range);
        _mkdir(cm->nemo_cfg->save_imgs_dir);
        
        char dim_file_path[PATH_MAX];
        strcpy(dim_file_path, cm->nemo_cfg->save_imgs_dir);
        FILE *dim_file = fopen(strcat(dim_file_path, "/width_height.txt"), "w");
        fprintf(dim_file, "%d\t%d", width, height);
        fclose(dim_file);
        
        if (cm->show_frame)
            sprintf(file_name, "%s/%04d.raw", cm->nemo_cfg->save_imgs_dir, cm->current_video_frame - 1);
        else
            sprintf(file_name, "%s/%04d_%d.raw", cm->nemo_cfg->save_imgs_dir, cm->current_video_frame, cm->current_super_frame);
        RGB24_save_frame_buffer(scaled_rgb_frame, file_name);
    }
}
