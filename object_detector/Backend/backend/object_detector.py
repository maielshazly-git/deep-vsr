...
class Detector:
    ...
    def __init__(self, num_classes=91,train_weights="/home/mai/nemo/object_detector/Backend/backend/fasterrcnn_resnet50_fpn_coco.pth"): # Mai, final task
    ...
    def infer(self,image):
        ...
        with torch.no_grad():
            ...
            for i in range(boxes.shape[0]):  # boxes  列表  n*4
                ...
                # print(x1, y1, x2, y2,"=======================bbox") # Mai, commented for the next line
                #box_tuple = (x1 / width, y1 / height, (x2 - x1) / width, (y2 - y1) / height)
                box_tuple = (x1, y1, x2, y2) # Mai, because I need the absolute values to work with tensors later for sr
                conf = scores[i]
